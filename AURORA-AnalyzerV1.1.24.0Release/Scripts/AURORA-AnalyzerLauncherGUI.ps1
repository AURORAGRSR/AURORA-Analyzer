...
# LauncherGUI Version: V1.1.19.8Release
# 作者：AURORA VelociRaptor-GR Dev PRJ.| 构建时间：2026.05.25
...
# 新增：RSA 令牌验证（EXE 启动的实际路径）
if (-not $IsLaunchedByExe) {
    $TokenPath = $env:AURORA_TOKEN_PATH
    Write-Host "[调试] AURORA_TOKEN_PATH = '$TokenPath'" -ForegroundColor Cyan
    
    if (-not [string]::IsNullOrWhiteSpace($TokenPath) -and (Test-Path $TokenPath)) {
        try {
            $tokenContent = Get-Content $TokenPath -Raw -Encoding UTF8
            Write-Host "[调试] 令牌文件内容长度：$($tokenContent.Length) 字符" -ForegroundColor Cyan
            
            # 使用正确的分割方式：分割为最多 4 部分
            $parts = $tokenContent -split ':'
            Write-Host "[调试] 分割后部分数：$($parts.Count)" -ForegroundColor Cyan
            
            if ($parts.Count -ge 4) {
                $tokenNonce = $parts[0]
                $tokenTimestamp = [long]$parts[1]
                # hashB64 可能很长，但不包含 ':'
                $tokenSignature = $parts[-1]  # 最后一部分是签名
                $tokenHashB64 = $parts[2..($parts.Count - 2)] -join ':'  # 中间部分是 hashB64
                
                Write-Host "[调试] Nonce: $tokenNonce" -ForegroundColor Cyan
                Write-Host "[调试] Timestamp: $tokenTimestamp" -ForegroundColor Cyan
                Write-Host "[调试] HashB64 长度：$($tokenHashB64.Length)" -ForegroundColor Cyan
                Write-Host "[调试] Signature 长度：$($tokenSignature.Length)" -ForegroundColor Cyan
                ...
                if (Test-RSATokenSignature -Nonce $tokenNonce -Timestamp $tokenTimestamp -HashPayload $tokenHashB64 -Signature $tokenSignature) {
                    ...
                    if ($age -lt 60 -and $age -gt -5) {
                        ...
                        if ($null -ne $decryptedHashList -and $decryptedHashList.Length -gt 0) {
                            ...
                            Write-Host "[错误] Hash 列表解密失败或为空" -ForegroundColor Red
                        }
                    } else {
                        Write-Host "令牌已过期（年龄：$age 秒）" -ForegroundColor Yellow
                    }
                } else {
                    Write-Host "RSA 签名验证失败 - 检测到伪造令牌" -ForegroundColor Red
                }
            } else {
                Write-Host "令牌格式错误（预期至少 4 部分，实际 $($parts.Count) 部分）" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "令牌验证失败：$($_.Exception.Message)" -ForegroundColor Red
        ...
        Write-Host "[调试] 未找到令牌文件或路径为空" -ForegroundColor Yellow
    }
}
...
function Initialize-RuntimeIntegrityCheck {
    ...
    $script:checkIntegrity = {
        ...
        if ($tampered) {
            ...
            # 正常检查，写入调试日志
            Write-Host "[完整性检查] 检查通过 - 共检查 $($script:ExpectedFileHashes.Count) 个文件" -ForegroundColor DarkGray
        }
    }
...
if ($null -ne $global:PassedHashListFromExe -and $global:PassedHashListFromExe.Length -gt 0) {
    # EXE 启动或密码验证成功路径：使用解密后的 hash 列表
    ...
    if (-not $integrityResult.Success) {
        $failedList = $integrityResult.FailedFiles -join "`n"
        $script:ExitCountdownStarted = $true
...
Add-Type -ReferencedAssemblies @("System.Drawing", "System.Windows.Forms", $AURORA_Engine_Assembly) -TypeDefinition $guiCsharpCode -Language CSharp
                ...
                try {
                    ...
                    if ($script:selectedMode -eq "SMART") {
                        ...
                        if (Test-Path $smartScript) {
                            # 💥 核心升级：传递 FromGUI 参数，启用 CSV 检测和全息对话框
                            if ($script:selectedLanguage -eq "ENG") {
                                ShowProMode -ScriptPath $smartScript -Title "Smart Auto-Healing" -Language "ENG" -FromGUI $true
                            } else {
                                ShowProMode -ScriptPath $smartScript -Title "智能诊断与自主修复模式" -Language "CHS" -FromGUI $true
                            }
                        }
