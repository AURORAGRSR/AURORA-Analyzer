# AURORA Analyzer V1.1.24.0 — 用户手册
...
- [8. v1.1.24.0 新功能](#8-v11240-新功能)
  - [8.1 安全防护全面升级](#81-安全防护全面升级)
  - [8.2 性能优化](#82-性能优化)
  - [8.3 稳定性修复](#83-稳定性修复)
- [9. 常见问题与场景](#9-常见问题与场景)
...
1. **解压** `AURORA-AnalyzerV1.1.24.0Release.zip` 到任意目录
...
v1.1.24.0 对安全体系进行了全面重构，核心变化包括：

#### 🛡️ 更安全的启动验证

- 采用银行级 RSA-2048 加密算法验证程序完整性
- 启动时自动检查所有核心文件是否被篡改
- 如果检测到文件被修改，程序将拒绝启动并给出警告

**这对您意味着什么:** 您下载的 AURORA Analyzer 不会被恶意软件悄悄修改或植入后门。

#### 🔍 运行时实时防护

- 程序运行期间，每 3 秒自动检查核心文件完整性
- 实时监控文件系统变化
- 检测到可疑修改后立即锁定程序并安全退出

**这对您意味着什么:** 您的工具在运行中也不会被任何人或程序篡改。

#### 🚫 反调试保护

- 自动检测并阻止 x64dbg、OllyDbg 等调试工具
- 防止恶意用户通过调试器分析或修改程序行为

**这对您意味着什么:** 增加了攻击者想要逆向工程或破解程序的难度，保护了您的使用安全。

#### 📂 未授权文件检测

- 启动时检查程序目录中是否存在不应该出现的文件
- 防止恶意文件被偷偷放入程序目录

**这对您意味着什么:** 如果有人试图在您的工具目录中植入病毒文件，程序会发现并拒绝启动。

### 8.2 性能优化

- **安全检测响应速度提升 70%**: 检查间隔从 10 秒缩短到 3 秒
- **启动后立即检查**: 不再有启动初期的安全真空期
- **全量文件检查**: 修复了之前可能遗漏部分文件的漏洞

### 8.3 稳定性修复

- 修复了完整性检查时可能导致部分文件漏检的 bug
- 优化了文件哈希计算的性能开销
- 改进了异常情况下的清理逻辑
