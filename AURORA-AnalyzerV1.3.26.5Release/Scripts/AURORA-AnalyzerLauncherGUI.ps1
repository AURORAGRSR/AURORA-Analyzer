    ...
    $script:fileWatcher.Add_Created({
        ...
        if (-not $script:ExpectedFileHashes.ContainsKey($relativePath)) {
            ...
            if ($global:proForm) {
                ...
                } catch { Write-AuroraLog "proForm 隐藏失败(文件监控): $($_.Exception.Message)" -Level "Warning" }
            }
            if ($splash) {
                try {
                    $splash.Hide()
                } catch { Write-AuroraLog "splash 隐藏失败(文件监控): $($_.Exception.Message)" -Level "Warning" }
            }
            if ($global:mainForm) {
                try {
                    if (-not $global:mainForm.IsDisposed) {
                ...
                } catch { Write-AuroraLog "mainForm 隐藏失败(文件监控): $($_.Exception.Message)" -Level "Warning" }
            }
