...
try {
    if ([System.Type]::GetType('AuroraGuard') -or $null -ne [AuroraGuard]) {
        $verifyResult = [AuroraGuard]::VerifyOrDie()
    ...
    # 类型不存在或验证抛异常：安全失败（fail-closed）
    # 🔒 M-3：不输出异常详情到控制台，防止泄露安全守卫内部实现
    Write-Host "[Security] AuroraGuard verification error" -ForegroundColor Red
    # 仅当类型确实不存在时才跳过（已在 LauncherGUI 中验证过）
    if (-not [System.Type]::GetType('AuroraGuard') -and $null -eq ([AuroraGuard] -as [type])) {
        Write-Host "[Security] AuroraGuard type not loaded, skipping (assumed pre-verified)." -ForegroundColor DarkGray
    } else {
        # 类型存在但调用失败，强制退出
        if (Get-Command Invoke-SafeExit -ErrorAction SilentlyContinue) {
            Invoke-SafeExit -ExitCode 1
