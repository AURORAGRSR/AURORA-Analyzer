        ...
        if ($restoredSession) {
            ...
            if ($GUI_Mode -or (Get-Variable -Name "syncHash" -Scope Global -ErrorAction SilentlyContinue)) {
                ...
                if ($null -ne $global:syncHash -and $global:syncHash -is [System.Collections.IDictionary]) {
                    ...
                    Write-Host "⚠️ syncHash 无效，使用控制台模式" -ForegroundColor Yellow
                    $restoreMode = $false
                }
            } else {
                # 非 GUI 模式：输出控制台选择提示
                Write-Host "⚠️ 检测到未完成的会话，控制台模式下假设重新开始" -ForegroundColor Yellow
                $restoreMode = $false
            }
...
try {
    ...
    if ($concurrentMode) {
        Write-Host "`n🚀 Phase 3.4 并发优化：检测到多个日志类型，启用并发处理模式..." -ForegroundColor Green
        Write-Host "   日志类型数量：$($scopes.Count)" -ForegroundColor Cyan
        Write-Host "   预计加速比：$([Math]::Min($scopes.Count, 3))x" -ForegroundColor Cyan
        
        # Phase 3.4 核心优化：并发执行多个日志类型的导出
        # 使用已有的 Invoke-ParallelTask 函数（L3757）
        $maxThreads = [Math]::Min($scopes.Count, 3)  # 最多 3 个并发线程
        Write-Host "   最大并发线程数：$maxThreads" -ForegroundColor Cyan
        ...
        Write-Host "✅ 并发处理完成" -ForegroundColor Green
    }
    ...
    for ($scopeIndex = 0; $scopeIndex -lt $scopes.Count; $scopeIndex++) {
        ...
        if ($concurrentMode -and $scope.ProcessedByParallelTask) {
            Write-Host "`n✅ 已跳过：$($scope.LogType)（已由并发任务处理）" -ForegroundColor Green
            continue
        }
