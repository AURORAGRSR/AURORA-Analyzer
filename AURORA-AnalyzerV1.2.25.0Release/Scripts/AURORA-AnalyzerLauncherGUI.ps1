...
# LauncherGUI Version: V1.2.22.0Release
# 作者：AURORA VelociRaptor-GR Dev PRJ.| 构建时间：2026.06.02
...
public class AuroraGuard
{
    private static readonly Dictionary<string, string> _expected = new Dictionary<string, string>
    {
        ...
        { "Scripts\\AURORA-AnalyzerPRO.ps1", "37c0303edfc057e3f4445732c2f5ef46ffd1b324f669cba2c77a49dfdede9ebb" },
        ...
        { "Scripts\\Core\\AURORA-AnimationCoreEngine.ps1", "5eb5d8cb1f5b3d50d7dd616c4eabe84ec354daf359869422aa6e35dd376b48a2" },
...
public class AuroraResultModal : Control
{
    ...
    protected override void OnPaint(PaintEventArgs e) {
        ...
        try { ApplyTransitionTransform(g); } catch { }
            ...
            $waitTimer.Add_Tick({
                ...
                try {
                    ...
                    if ($null -eq $Global:MagneticRestoreTimer -or -not $Global:MagneticRestoreTimer.Enabled) {
                        $Global:MagneticRestoreTimer = New-Object System.Windows.Forms.Timer
        ...
        if (-not $script:launched -and -not $Global:IsViewTransitioning) {
            ...
            $waitTimer.Add_Tick({
                ...
                try {
                    ...
                    $restoreTimer2 = New-Object System.Windows.Forms.Timer
                    $restoreTimer2.Interval = 650
                    $restoreTimer2.Add_Tick({
                        $this.Stop()
                        $this.Dispose()
                        $btnSmart.SetTransitionMode($false)
                        $btnEng.SetTransitionMode($false)
        ...
        if (-not $script:launched -and -not $Global:IsViewTransitioning) {
            ...
            $waitTimer.Add_Tick({
                ...
                try {
                    ...
                    # 关键修复：延迟恢复磁吸效果，等待飞入动画完成后再启用
                    # 最长动画：350ms + 200ms 延迟 = 550ms，额外加 100ms 缓冲 = 650ms
                    $magneticRestoreTimer = New-Object System.Windows.Forms.Timer
            ...
            # ==========================================
                ...
                $waitTimer.Add_Tick({
                    ...
                    try {
                        ...
                        $Global:IsViewTransitioning = $false
                        # 关键修复：延迟恢复磁吸效果，等待飞入动画完成后启用
                        $restoreTimer5 = New-Object System.Windows.Forms.Timer
