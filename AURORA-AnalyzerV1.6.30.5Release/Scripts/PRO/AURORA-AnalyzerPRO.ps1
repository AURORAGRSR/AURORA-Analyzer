...
# ==========================================
# [LANG-FIX] dot-source 必须显式传递 -Language，否则 CoreEngine.ps1 的 param 块
#   param([string]$Language = "CHS") 会用默认值 "CHS" 覆盖 PRO.ps1 param 块解析的 $Language
#   （PowerShell dot-source 调用时，子脚本 param 块的默认值会覆盖父作用域同名变量）
#   实测复现：parent $Language="ENG" → . child.ps1 (无参) → parent $Language="CHS"
#   影响：GUI 选 ENG 后 PRO 收到 -Language ENG，但 dot-source 后 $Language 被重置为 CHS，
#         导致 [PRO] Starting AURORA PRO Mode (Language: CHS) 与 SmartEngine 全程中文。
...
if ($ConsoleMode) {
    ...
    $consoleChoice = $null
    try {
        $prompt = if ($Language -eq "ENG") { "Select mode (1/2) [default: 2=Pro Mode]" } else { "请选择模式 (1/2) [默认: 2=专业模式]" }
        $consoleChoice = Read-Host $prompt
    } catch {
        # 非交互环境，使用默认值
        $consoleChoice = ""
    }
    ...
    if ($consoleChoice -eq "1") {
        ...
        if (Test-Path $smartCliPath) {
            ...
            # [SEC-CLI-PASS] 设置进程内令牌：SmartEngine-CLI.ps1 守卫需要 RSA 令牌或父进程名
            #   验证。但 EXE 启动 powershell 后 Shutdown，PRO 通过 & 同进程调用 CLI 时
            #   父进程已不存在，RSA 令牌又仅由 EXE 私钥签发（PS 端无法自行签发）。
            #   令牌作用域仅限当前 PowerShell 进程内（& 调用同 runspace），外部启动时为空。
            $global:AURORA_PRO_VERIFIED = $true
