...
# ==========================================
# 🌐 CLI 双语化辅助（_GL = Guard Log localize）
# ==========================================
function _GL {
...
# [SEC-CLI-PASS] 进程内令牌：PRO.ps1 通过 & 调用本脚本时设置
#   作用域仅限当前 PowerShell 进程（同 runspace），外部启动 powershell.exe 时该变量为空。
#   PRO.ps1 通过自身守卫（GUI_Mode 参数 / RSA 令牌）后才能到达此处设置令牌，
#   等同于信任 PRO.ps1 的启动合法性。EXE Shutdown 后父进程已退出，父进程名校验不可用，
#   RSA 令牌又仅由 EXE 私钥签发，PS 端无法自行签发——这是 & 调用场景的唯一可行通路。
$proVerified = $null
if ($FromPRO) {
    $proVerified = Get-Variable -Name "AURORA_PRO_VERIFIED" -Scope Global -ValueOnly -ErrorAction SilentlyContinue
}
if (-not $isLaunchedLegitimately -and $FromPRO -and ($true -eq $proVerified)) {
    $isLaunchedLegitimately = $true
    Write-Host "[SmartEngine-CLI] Launched by PRO script (in-process token)" -ForegroundColor DarkGray
}
...
try {
    while (-not $asyncResult.IsCompleted) {
        ...
        if ($global:syncHash.RequiresElevation -eq $true) {
            ...
            Write-Host (_GL "  需要管理员权限" "  Administrator privileges required") -ForegroundColor Yellow
            Write-Host "========================================" -ForegroundColor Yellow
            if ($global:syncHash.ElevationReason) {
                Write-Host "  $($global:syncHash.ElevationReason)" -ForegroundColor White
            }
            Write-Host "----------------------------------------" -ForegroundColor Gray
            Write-Host (_GL "  [y] 以管理员身份重启  [n] 跳过此操作" "  [y] Restart as admin  [n] Skip this operation") -ForegroundColor White
            $choice = Read-Host (_GL "请选择" "Select")
            ...
            if ($choice -eq 'y') {
                ...
                try {
                    ...
                    Write-Host (_GL "  请手动以管理员身份运行。" "  Please run manually as administrator.") -ForegroundColor Yellow
                }
        ...
        if ($global:syncHash.RequiresAuthorization -eq $true -and $null -eq $global:syncHash.Authorized) {
            ...
            Write-Host ""
            Write-Host "========================================" -ForegroundColor Red
            Write-Host (_GL "  高危操作授权确认" "  High-Risk Operation Authorization") -ForegroundColor Red
            Write-Host "========================================" -ForegroundColor Red
            Write-Host (_GL "  操作: $cmdName" "  Operation: $cmdName") -ForegroundColor White
            if ($riskLevel) {
                $riskColor = "Green"
                ...
                Write-Host (_GL "  风险等级: $riskLevel" "  Risk Level: $riskLevel") -ForegroundColor $riskColor
            }
            if ($description) {
                Write-Host (_GL "  说明: $description" "  Description: $description") -ForegroundColor Gray
            }
            Write-Host "----------------------------------------" -ForegroundColor Gray
            Write-Host (_GL "  [y] 确认执行  [n] 拒绝  [s] 跳过" "  [y] Confirm  [n] Reject  [s] Skip") -ForegroundColor White
            $choice = Read-Host (_GL "请选择" "Select")
            ...
            switch ($choice.ToLower()) {
                'y' {
                    ...
                    # [SEC-AUTH-FIX] 不要清空 PendingCommand：
                    #   Invoke-AuroraSafeAction 在 EventWaitHandle 醒来后只 Write-SmartLog 然后第 654 行
                    #   `return "PendingAuthorization"` 提前返回——命令并未执行。
                    #   实际执行依赖 SmartEngine 主循环分支 1 (Authorized=true AND PendingCommand!=null)
                    #   克隆命令 + 设 auto_execute=true 后重新调用 Invoke-AuroraSafeAction 才会真执行。
                    #   原注释"防止双重执行"判断错误——分支 1 不是双重执行，是唯一的执行入口。
                    #   之前清空 PendingCommand 导致分支 1 条件不成立 → 主循环死循环 → 控制台卡住。
                ...
                default {
                    $global:syncHash.Authorized = $false
                    $global:syncHash.PendingCommand = $null
                    # [SEC-AUTH-FIX] 'n'/拒绝分支也需重置 RequiresAuthorization=false，
                    #   触发 SmartEngine 分支 2 (RequiresAuthorization=false AND PendingCommand=null) 重置。
                    #   否则 SmartEngine 主循环所有分支均不成立 → 死循环。
                    $global:syncHash.RequiresAuthorization = $false
                    $authRejected = $true
                }
            }
            ...
            if ($waitHandle) {
                # [FIX] Set()/Close() 返回 bool，PowerShell try 块内默认送入管道，
                #   会把 "True" 打印到控制台（污染输出）。用 $null = 抑制管道输出。
                try { $null = $waitHandle.Set(); $null = $waitHandle.Close() } catch {}
            }
        ...
        if ($global:syncHash.RequiresUserInput -eq $true) {
            ...
            if ($inputType -eq "UseExportedLogs") {
                Write-Host (_GL "  检测到已导出的 CSV 日志" "  Exported CSV logs detected") -ForegroundColor Cyan
                Write-Host "========================================" -ForegroundColor Cyan
                if ($inputData) {
                    Write-Host (_GL "  文件数: $($inputData.FileCount)" "  File count: $($inputData.FileCount)") -ForegroundColor White
                    Write-Host (_GL "  导出路径: $($inputData.ExportPath)" "  Export path: $($inputData.ExportPath)") -ForegroundColor Gray
                    if ($inputData.FileAge) {
                        Write-Host (_GL "  最新文件时间: $($inputData.FileAge)" "  Latest file time: $($inputData.FileAge)") -ForegroundColor Gray
                    }
                }
                Write-Host "----------------------------------------" -ForegroundColor Gray
                Write-Host (_GL "  [y] 使用已导出的日志  [n] 重新扫描" "  [y] Use exported logs  [n] Rescan") -ForegroundColor White
                $choice = Read-Host (_GL "请选择" "Select")
                if ($choice -eq 'y') {
                ...
                Write-Host (_GL "  需要用户输入 ($inputType)" "  User input required ($inputType)") -ForegroundColor Cyan
                Write-Host "========================================" -ForegroundColor Cyan
                $choice = Read-Host (_GL "请输入" "Enter")
        ...
        if (-not $commandExecuting -and $global:syncHash.SmartMenuItems -and -not $menuPrinted) {
            ...
            if ($global:syncHash.RequiresAuthorization -ne $true -and
                ...
                if ($Language -eq "CHS") {
                    ...
                    Write-Host "  Tip: Enter number to repair | Enter to exit | m to refresh menu" -ForegroundColor Gray
                }
                Write-Host "========================================" -ForegroundColor Cyan
                Write-Host ""
