...
if (Test-Path $launchGuardPath) {
    . $launchGuardPath
    # [V1.5.29.1 修复] 对齐 PRO-Engine 的防御性调用：先检查函数是否存在再调用，
    #   避免 Runspace 复用场景下 dot-source 作用域异常导致 CommandNotFoundException。
    #   根因：ProMode 终止 PRO Engine 后，Runspace 全局变量 $AURORA_LaunchGuard_Loaded=true 残留，
    #   虽然 LaunchGuard.ps1 已移除 return 防止跳过函数定义，但 Runspace 状态污染可能导致
    #   dot-source 不完整执行。Get-Command 检查是最后防线，确保不会因函数缺失而中断流程。
    if (Get-Command Assert-AuroraLaunchContext -ErrorAction SilentlyContinue) {
        ...
        Write-Warning "Assert-AuroraLaunchContext not available after dot-source LaunchGuard, skipping launch context validation"
    }
} else {
...
# ==========================================
# [P0-修复] 当 SmartEngine 被直接调用（智能模式直连，而非经 PRO Engine 内部调用）时，
#   SecurityModule.ps1 可能尚未被 dot-source（AuroraGuard 类型不存在）。此处按需加载。
#   SecurityModule 内部已有"类型已存在"的兼容处理（L887-981），重复 dot-source 安全。
# [P0-修复] 设置 $AuroraGuardSkipMonitoring=true 跳过持续性定时器和 WMI 监控启动：
#   SmartEngine 是"短跑型"脚本，dot-source 后立即独占 Runspace 执行分析，
#   定时器事件回调需要借用 Runspace 执行脚本块，与主流程抢占 RunspacePool，
#   导致 ProModeViewModel.IsAlive(2000) 超时失败（误报"主动健康检查失败"循环）。
#   VerifyOrDie 静态验证仍保留，启动时的环境扫描也仍执行，只跳过"持续性"资源。
# [P0-修复] 重要：用 [AuroraGuard] -as [type] 做类型存在性检测（包在 try/catch 中），
#   而非 [System.Type]::GetType('AuroraGuard')。后者对 Add-Type 生成的全局类型返回 $null
#   （GetType 需要完全限定名或程序集名），导致即使类型真实存在也判断为"未加载"，
#   误触发重复 dot-source 后又因 GetType 仍返回 null 进入 fail-closed 退出分支。
function Get-AuroraGuardType {
    try {
        return [AuroraGuard] -as [type]
    } catch {
        return $null
    }
}

$AuroraGuardSkipMonitoring = $true
if ($null -eq (Get-AuroraGuardType)) {
    $securityModulePath = Join-Path (Join-Path $parentDir "Security") "AURORA-SecurityModule.ps1"
    if (Test-Path $securityModulePath) {
        . $securityModulePath
    } else {
        Write-Warning "SecurityModule not found at: $securityModulePath"
    }
}
Remove-Variable AuroraGuardSkipMonitoring -ErrorAction SilentlyContinue

# ==========================================
# 📦 加载 CoreEngine（提供 Invoke-SafeExit 等核心函数）
# ==========================================
# [SEC-RUNSPACE-FIX] 必须在 SmartEngine 自己的执行上下文中 dot-source CoreEngine：
#   - SmartEngine 经 PRO 调用时，PRO 已 dot-source CoreEngine，$global:AURORA_CoreEngine_Loaded=true
#     此处 dot-source 命中 CoreEngine L20-23 的"已加载则跳过"机制，无副作用。
#   - SmartEngine 经 CLI 适配层调用时，CLI 用 RunspaceFactory::CreateRunspace() 创建独立 Runspace，
#     新 Runspace 不继承父 Runspace 的全局函数表——$global:AURORA_CoreEngine_Loaded 在新 Runspace 中为 $null，
#     必须在此处真正加载，否则 SmartEngine 后续多处直接调用 Invoke-SafeExit（L174/187/208/547/961）
#     会抛 CommandNotFoundException（用户场景：管理员提权后 ElevationAuthorized=true 触发 L547 退出
#     时复现"无法将 Invoke-SafeExit 项识别为 cmdlet"致命错误）。
$coreEnginePath = Join-Path (Join-Path $parentDir "Core") "AURORA-CoreEngine.ps1"
if (Test-Path $coreEnginePath) {
    . $coreEnginePath -Language $Language
} else {
    Write-Warning "CoreEngine not found at: $coreEnginePath (Invoke-SafeExit will be unavailable)"
}

# 🔒 安全修复 C-3：VerifyOrDie 返回 false 时必须强制退出，不能静默跳过
try {
    $guardType = Get-AuroraGuardType
    if ($null -ne $guardType) {
        $verifyResult = $guardType::VerifyOrDie()
        if ($verifyResult -ne $true) {
            Write-Host "[Security] Integrity check failed. Aborting." -ForegroundColor Red
        ...
        # 类型仍不存在：SecurityModule 加载失败，安全失败（fail-closed）
        Write-Host "[Security] AuroraGuard type not loaded after SecurityModule dot-source, aborting." -ForegroundColor Red
        if (Get-Command Invoke-SafeExit -ErrorAction SilentlyContinue) {
            Invoke-SafeExit -ExitCode 1
        } else {
            [Environment]::Exit(1)
    ...
    # 🔒 M-3：不输出异常详情到控制台，防止泄露安全守卫内部实现
    Write-Host "[Security] AuroraGuard verification error" -ForegroundColor Red
    # 类型存在但调用失败，强制退出
    if (Get-Command Invoke-SafeExit -ErrorAction SilentlyContinue) {
        Invoke-SafeExit -ExitCode 1
    } else {
...
# 🔒 H-6 / C-2：命令内容安全校验
# 检查 JSON 命令内容是否包含明显恶意的模式
# 注意：命令来源已通过完整性校验（哈希清单），此处为纵深防御
function Test-AuroraCommandSafe {
    param([string]$CommandStr)
    if ([string]::IsNullOrWhiteSpace($CommandStr)) { return $true }

    # 🔒 C-2：命令长度限制（防止超长命令注入或资源耗尽）
    $maxCommandLength = 8192
    if ($CommandStr.Length -gt $maxCommandLength) {
        Write-AuroraLog "Command blocked: exceeds max length ($($CommandStr.Length) > $maxCommandLength)" -Level "Warning" -Source "Security"
        return $false
    }

    # 🔒 C-2：增强黑名单：拒绝危险 cmdlet、绕过尝试和代码注入模式
    $dangerousPatterns = @(
        # 命令执行类
        'Invoke-Expression\b',
        '\biex\b',
        ...
        '-Enc\b',
        'FromBase64String\b',
        'ToBase64String\b.*\bFromBase64String\b',
        # 字符串拼接绕过检测
        '\[char\]\s*\d',
        '\[convert\]::ToInt32\s*\(\s*['']',
        # 进程注入类
        'PowerShell\b.*-Command\b',
        'PowerShell\b.*-EncodedCommand\b',
        'cmd\b.*/c\b',
        'certutil\b.*-decode\b',
        'certutil\b.*-urlcache\b',
        'bitsadmin\b.*/transfer\b',
        'mshta\b',
        'wscript\b.*\.vbs',
        'cscript\b.*\.vbs',
        # WMI/CIM 绕过
        ...
        'Remove-Item\b.*HKLM:\\SOFTWARE\\Microsoft\\Windows',
        # 环境变量篡改（针对 AURORA 自身）
        'AURORA_PublicKeyXml',
        'AURORA_SessionSalt',
        'AURORA_AesSalt',
        'env:AURORA_TOKEN_PATH',
        'env:AURORA_HASH_PATH'
    )
    foreach ($pat in $dangerousPatterns) {
        if ($CommandStr -match $pat) {
            Write-AuroraLog "Command blocked: matched dangerous pattern" -Level "Warning" -Source "Security"
            return $false
        }
    }
    return $true
...
# 🔒 H-6 / C-2：安全执行命令（替代 Invoke-Expression，使用 ScriptBlock 限定作用域）
# 使用 ScriptBlock 在当前作用域执行，避免 Invoke-Expression 的字符串解析阶段
# 🔒 C-2 加固：添加执行超时、审计日志
function Invoke-AuroraSafeCommand {
    param(
        [string]$CommandStr,
        [switch]$CaptureError
    )
    if (-not (Test-AuroraCommandSafe $CommandStr)) {
        throw "Command blocked by safety check (H-6/C-2)"
    }

    # 🔒 C-2：审计日志 — 记录所有执行的命令到安全日志
    try {
        $auditDir = Join-Path $env:LOCALAPPDATA "AURORA\Logs"
        if (-not (Test-Path $auditDir)) {
            New-Item -ItemType Directory -Path $auditDir -Force | Out-Null
        }
        $auditFile = Join-Path $auditDir "command_audit.log"
        $auditEntry = "[{0}] PID={1} CmdLen={2} Hash={3}" -f `
            (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'),
            $PID,
            $CommandStr.Length,
            ([System.Security.Cryptography.SHA256]::Create().ComputeHash(
                [System.Text.Encoding]::UTF8.GetBytes($CommandStr)) | ForEach-Object { $_.ToString('x2') }) -join ''
        # 截取命令前 200 字符用于审计（不记录完整命令，防止泄露敏感参数）
        $auditPreview = if ($CommandStr.Length -gt 200) { $CommandStr.Substring(0, 200) + '...' } else { $CommandStr }
        $auditEntry += " Preview={0}" -f $auditPreview
        Add-Content -Path $auditFile -Value $auditEntry -Encoding UTF8
    } catch {}

    $sb = [ScriptBlock]::Create($CommandStr)

    # 🔒 C-2：在当前作用域执行（命令需要访问 $global:syncHash 和已加载模块）
    # 注意：不使用 Start-Job（会隔离作用域，导致命令无法访问会话上下文）
    if ($CaptureError) {
        return (& $sb 2>&1)
    } else {
        return (& $sb)
...
if (-not (Get-Command Create-BackupSnapshot -ErrorAction SilentlyContinue)) {
    ...
    if ($Command.pre_check -and $Command.pre_check -ne "") {
        try {
            # 🔒 H-6：使用安全执行函数替代 Invoke-Expression，防止命令注入
            $preCheckResult = Invoke-AuroraSafeCommand -CommandStr $Command.pre_check
    ...
    if ($needsAuth) {
        ...
        # 🔒 安全修复 H-2：使用随机 GUID 替代可预测的 $PID，防止外部进程枚举 PID 后构造事件名绕过授权
        $eventGuid = [System.Guid]::NewGuid().ToString('N')
        $eventName = "AURORA_Auth_$eventGuid"
        ...
        try {
            ...
            # 如果事件不存在，创建一个新的
            # 🔒 M-7：使用自定义安全描述符，仅允许当前用户进程同步和修改
            # 默认安全描述符允许同用户所有进程 OpenExisting 并 Set() 事件
            $createdNew = $false
            $evtSecurity = New-Object System.Security.AccessControl.EventWaitHandleSecurity
            $evtRule = New-Object System.Security.AccessControl.EventWaitHandleAccessRule(
                [System.Security.Principal.WindowsIdentity]::GetCurrent().User,
                "FullControl", "Allow")
            $evtSecurity.AddAccessRule($evtRule)
            $evtSecurity.SetAccessRuleProtection($true, $false)
            $waitHandle = New-Object System.Threading.EventWaitHandle(
                $false, [System.Threading.EventResetMode]::ManualReset, $eventName,
                [ref]$createdNew, $evtSecurity)
        }
        
        # 阻塞等待（最多 30 秒），不消耗 CPU
        $waitStartMs = [System.Diagnostics.Stopwatch]::GetTimestamp()
        $signaled = $waitHandle.WaitOne(30000)
        $waitEndMs = [System.Diagnostics.Stopwatch]::GetTimestamp()
        # [FIX] 计算实际等待毫秒数。原代码 -f "EventWaitHandle" 把字面字符串填入
        #   "等待了 {0}ms" 占位符，输出 "等待了 EventWaitHandlems"（语法断裂）。
        $elapsedMs = [int]([long]$waitEndMs - [long]$waitStartMs) / [System.Diagnostics.Stopwatch]::Frequency * 1000
    ...
    if ($isRepairCommand -and $hasSideEffects) {
        ...
        Write-SmartLog ""
        Write-SmartLog $L["Undo_Creating"]
        ...
        try {
            ...
            if ($repairSession) {
                ...
                Write-SmartLog ($L["Undo_Session_Created"] -f $repairSessionId)
            }
            ...
            if ($needsBackup) {
                # 提取命令中的路径信息
                $paths = @()
                # 🔒 安全修复 M-1：路径正则改进，支持含空格路径和带引号路径
                # 原正则 [^"\s\|]+ 遇到空格即截断，导致 C:\Program Files\... 被截断为 C:\Program
                
                # 匹配注册表路径：HKLM:\xxx, HKCU:\xxx, HKU:\xxx（允许空格，遇到引号或管道符停止）
                $regMatches = [regex]::Matches($Command.command, '(HK[LMU]:\\[^"|]+)')
                foreach ($m in $regMatches) { $paths += $m.Value.Trim() }
                
                # 匹配文件路径：C:\xxx, D:\xxx 等（允许空格，遇到引号或管道符停止）
                $fileMatches = [regex]::Matches($Command.command, '([A-Za-z]:\\[^"|]+)')
                foreach ($m in $fileMatches) { $paths += $m.Value.Trim() }
                ...
                if ($paths.Count -gt 0) {
                    ...
                    if ($snapshot) {
                        ...
                        Write-SmartLog ($L["Undo_Backup_Created"] -f $backupSnapshotId, $snapshot.Size)
                    }
                }
            }
    ...
    try {
        ...
        if ($Command.type -eq "powershell") {
            ...
            # 🔒 H-6：使用安全执行函数替代 Invoke-Expression，捕获错误流
            $executionOutput = (Invoke-AuroraSafeCommand -CommandStr $Command.command -CaptureError) | Out-String
        } else {
    ...
    if (-not $executionSuccess -and $Command.rollback_command -and $Command.rollback_command -ne "") {
        ...
        try {
            ...
            # 🔒 H-6：使用安全执行函数替代 Invoke-Expression，捕获错误流
            $rollbackOutput = Invoke-AuroraSafeCommand -CommandStr $Command.rollback_command -CaptureError
            ...
            Write-SmartLog ($lang["RollbackFail"])
...
$langDict = @{
    "CHS" = @{
        ...
        "Crash_Trace"   = "⚠️ 侦测到近期硬崩溃 (Event {0}). 靶向锁定: 崩溃溯源 ({1} 起)";
        "Boot_Check"    = "ℹ️ 近期重启，未发现崩溃特征。靶向锁定: 启动健康校验";
        ...
        "Custom_Range"  = "🎯 检测到用户指定时间范围: {0} ~ {1}";
        "LogTypes_Analyzed" = "📂 分析的日志类型：{0}";
        "Missing_Logs"  = "⚠️ 警告：未找到以下日志文件 - {0}";
        "Missing_Logs_Hint" = "💡 提示：请在 PRO 模式中也导出这些日志以获得完整分析";
        "CSV_Fallback_Msg" = "⚠️ PRO 导出的 CSV 数据不完整（仅 {0} 条事件），回退到实时提取模式";
        "Undo_Creating" = "📦 Phase 4.2: 正在创建撤销保护（Undo Support）...";
        "Undo_Session_Created" = "   ✅ 修复会话已创建：{0}";
        "Undo_Backup_Created" = "   ✅ 快速备份已创建：{0} (大小：{1})";
        "Undo_Ready"    = "   🔄 撤销保护已就绪（可撤销此修复操作）";
        "Undo_Failed"   = "   ⚠️ 撤销保护创建失败";
        "Mini_Found"    = "📋 发现 {0} 个蓝屏转储文件，正在解析关键信息...";
        "Mini_File_Info" = "  📋 {0} ({1} 天前):";
        "Mini_Name"     = "     名称：{0}";
        "Mini_Desc"     = "     说明：{0}";
        "Mini_Params"   = "     参数：{0}";
        "Mini_Solution" = "     💡 建议：{0}";
        "Mini_Parse_Error" = "  ⚠️ 无法解析 {0}: {1}";
        "Mini_Done"     = "✅ Minidump 分析完成，共分析 {0} 个文件";
        "GUI_Auth_Wait" = "⚠️ 请先在弹出的控制台上完成高危操作的授权决策！";
        "GUI_Menu_Showed" = "💡 修复菜单已在下方面板显示，可直接点击执行";
        # PRO 模式相关
    ...
    "ENG" = @{
        ...
        "Crash_Trace"   = "⚠️ Recent hard crash detected (Event {0}). Target: Crash Trace (from {1})";
        "Boot_Check"    = "ℹ️ Recent reboot, no crash signature found. Target: Boot Health Check";
        ...
        "Custom_Range"  = "🎯 Custom time range detected: {0} ~ {1}";
        "LogTypes_Analyzed" = "📂 Log types analyzed: {0}";
        "Missing_Logs"  = "⚠️ Warning: Following log files not found - {0}";
        "Missing_Logs_Hint" = "💡 Tip: Export these logs in PRO mode for complete analysis";
        "CSV_Fallback_Msg" = "⚠️ PRO exported CSV data incomplete (only {0} events), falling back to live extraction";
        "Undo_Creating" = "📦 Phase 4.2: Creating undo protection (Undo Support)...";
        "Undo_Session_Created" = "   ✅ Repair session created: {0}";
        "Undo_Backup_Created" = "   ✅ Quick backup created: {0} (size: {1})";
        "Undo_Ready"    = "   🔄 Undo protection ready (this repair operation can be undone)";
        "Undo_Failed"   = "   ⚠️ Undo protection creation failed";
        "Mini_Found"    = "📋 Found {0} minidump files, parsing key info...";
        "Mini_File_Info" = "  📋 {0} ({1} days ago):";
        "Mini_Name"     = "     Name: {0}";
        "Mini_Desc"     = "     Description: {0}";
        "Mini_Params"   = "     Parameters: {0}";
        "Mini_Solution" = "     💡 Suggestion: {0}";
        "Mini_Parse_Error" = "  ⚠️ Cannot parse {0}: {1}";
        "Mini_Done"     = "✅ Minidump analysis complete, {0} files analyzed";
        "GUI_Auth_Wait" = "⚠️ Please complete high-risk operation authorization in the popup console first!";
        "GUI_Menu_Showed" = "💡 Repair menu shown in panel below, click to execute";
        # PRO Mode Related
...
try {
    ...
    if ($null -ne $StartTime -and $null -ne $EndTime) {
        ...
        Write-SmartLog ($L["Custom_Range"] -f $realStartTime.ToString('MM-dd HH:mm'), $realEndTime.ToString('HH:mm')) "Custom Range"
    } 
    else {
        ...
        if ($lastCrash -and ($realEndTime - $lastCrash.TimeCreated).TotalHours -lt 48) {
            ...
            Write-SmartLog ($L["Crash_Trace"] -f $lastCrash.Id, $realStartTime.ToString('MM-dd HH:mm')) "Crash Trace Mode"
        }
        elseif ($uptime.TotalHours -lt 2) {
            # 刚开机不久，且近期无崩溃记录，往前推 4 小时以防万一
            $realStartTime = $realEndTime.AddHours(-4)
            Write-SmartLog $L["Boot_Check"] "Boot Check Mode"
        } 
        else {
            # 稳定运行中，执行常规 24 小时巡检
    ...
    if (Test-Path $minidumpPath) {
        ...
        if ($dmpFiles.Count -gt 0) {
            Write-SmartLog ($L["Mini_Found"] -f $dmpFiles.Count) "Minidump Analysis"
            ...
            foreach ($dmp in $dmpFiles) {
                try {
                    ...
                    if ($signature -eq 0x504D444D) {  # 验证签名
                        ...
                        Write-SmartLog ($L["Mini_File_Info"] -f $dmp.Name, $ageInDays)
                        Write-SmartLog "     BugCheck Code: 0x$($bugcheckCode.ToString("X8"))"
                        Write-SmartLog ($L["Mini_Name"] -f $bugcheckInfo.Name)
                        Write-SmartLog ($L["Mini_Desc"] -f $bugcheckInfo.Description)
                        $paramsStr = "0x$($bugcheckParameter1.ToString('X16')), 0x$($bugcheckParameter2.ToString('X16')), ..."
                        Write-SmartLog ($L["Mini_Params"] -f $paramsStr)
                        
                        # 如果 BugCheck 有已知解决方案，给出建议
                        if ($bugcheckInfo.Solution) {
                            Write-SmartLog ($L["Mini_Solution"] -f $bugcheckInfo.Solution)
                        }
                    }
                    ...
                    Write-SmartLog ($L["Mini_Parse_Error"] -f $dmp.Name, $_.Exception.Message)
                }
            }
            
            Write-SmartLog ($L["Mini_Done"] -f $dmpFiles.Count)
        }
    }
    ...
    if ($FromPRO -or $FromGUI) {
        ...
        try {
            ...
            if ($csvFiles.Count -gt 0) {
                ...
                # 报告已找到的日志类型
                $logTypeSeparator = if ($Language -eq "CHS") { '、' } else { ', ' }
                Write-SmartLog ($L["LogTypes_Analyzed"] -f ($foundLogTypes -join $logTypeSeparator)) "Log Types"
                ...
                if ($missingLogs.Count -gt 0) {
                    if ($Language -eq "CHS") {
                        ...
                        $missingLogNames = $missingLogs
                    }
                    $logTypeSeparator = if ($Language -eq "CHS") { '、' } else { ', ' }
                    Write-SmartLog ($L["Missing_Logs"] -f ($missingLogNames -join $logTypeSeparator)) "Missing Logs"
                    Write-SmartLog $L["Missing_Logs_Hint"] "Hint"
                }
    ...
    if ($rawEvents.Count -lt 10) {
        if ($rawEvents.Count -gt 0) {
            Write-SmartLog ($L["CSV_Fallback_Msg"] -f $rawEvents.Count) "CSV Fallback"
        }
    ...
    # [修复] 用 @() 强制数组化，避免 $matchedRulesMap 为空时 $matchedRules 为 $null，
    ...
    if ($matchedRules.Count -eq 0) {
        ...
        # [P0-修复] 无规则匹配时直接把进度推到 100，避免进度条卡在 Phase 4 入口的 80%。
        #   原代码该分支没有设 Progress=100，且不会进入 L1821 的 Progress=100（在 else 分支里）。
        ...
        while ($global:syncHash.IsHostAlive) {
            ...
            if ($global:syncHash.Authorized -eq $true -and $global:syncHash.PendingCommand) {
                ...
                # 🔒 安全修复 M-2：克隆命令对象再设置 auto_execute，避免变异原对象导致二次执行跳过授权
                # 原缺陷：直接对 $targetCmd（来自 $commandMap 的引用）设置 auto_execute=true，
                #         后续相同序号命令再次执行时会跳过授权
            ...
            if ($global:syncHash.UserInput -ne $null) {
                ...
                if ($global:syncHash.RequiresAuthorization -eq $true) {
                    Write-SmartLog $L["GUI_Auth_Wait"]
                ...
                if ($inputRaw -match "^(?i)m$") {
                    ...
                    Write-SmartLog $L["GUI_Menu_Showed"]
                    continue
                }
    ...
    #   导致 SmartEngine 在异常路径下进度条卡在最后写入的值（如 50/80）。
    #   即使 C# OnScriptComplete() 也会推 Progress=100，但有时 ScriptDone 信号
    #   读取有滞后，先在脚本端立即推 100 可让 UI 即时刷新。
    $global:syncHash.Progress = 100
    $global:syncHash.ScriptDone = $true
    $global:syncHash.IsRunning = $false
}
