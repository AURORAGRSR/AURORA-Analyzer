...
.NOTES
    Version: V12.1Release
    Author: AURORA VelociRaptor-GR Dev PRJ.
    Build Time: 2026.05.09
#>
Param(
    [string]$OutputPath,
...
# === Helper Function: Safe Progress Save ===
function Save-ProgressSafe {
    ...
    try {
        ...
        if ($sessionId) {
            ...
            Save-ENGProgress @params
        }
    }
    catch {
        Write-Verbose "[ProgressManager] Failed to save progress: $_"
    }
}

# === On-Demand Elevation Check Functions ===
...
function Invoke-ElevationCheck {
    ...
    # No privileges, trigger GUI elevation request
    if (Get-Variable -Name "syncHash" -Scope Global -ErrorAction SilentlyContinue) {
        # GUI mode: Set elevation request flag
        $global:syncHash.RequestElevation = $true
        $global:syncHash.ElevationReason = "$FeatureName requires administrator privileges to access $($LogType) log"
        $global:syncHash.ElevationLogType = $LogType
        $global:syncHash.ElevationAuthorized = $null
        
        Write-Host "⚠️ Accessing $($LogType) log requires administrator privileges" -ForegroundColor Yellow
        ...
        if ($global:syncHash.ElevationAuthorized -eq $true) {
            Write-Host "✅ User authorized elevation, restarting..." -ForegroundColor Green
            return $true
        } else {
            Write-Host "❌ User declined elevation, skipping $($LogType) log access" -ForegroundColor Red
            return $false
        }
    } else {
        # Console mode: Direct prompt
        Write-Host "⚠️ Accessing $($LogType) log requires administrator privileges" -ForegroundColor Yellow
        Write-Host "   Please re-run this tool as administrator" -ForegroundColor Cyan
        return $false
    }
}
            ...
            if ($dataCount -gt 10000) {
                ...
                foreach ($chunk in $chunks) {
                    ...
                    $powershell.AddScript({ param($data, $level) 
                        . "$PSScriptRoot\AURORA-AnalyzerENGPRO.ps1"
