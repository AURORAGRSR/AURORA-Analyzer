...
# 导入 CoreEngine
$coreEnginePath = Join-Path $scriptDir "AURORA-CoreEngine.ps1"
    ...
    switch ($RepairType) {
        ...
        "CleanSystem" {
            ...
            if (Test-Path $tempPath) {
                Get-ChildItem -Path $tempPath -Recurse -Force | Remove-Force -Recurse -ErrorAction SilentlyContinue
...
# 记录修复命令
if ($repairSuccess) {
    Log-RepairCommand -SessionId $session.SessionId -CommandType $RepairType -Parameters @{ Target = $Target } -Success
} else {
    Log-RepairCommand -SessionId $session.SessionId -CommandType $RepairType -Parameters @{ Target = $Target } -Success:$false
}
