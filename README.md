...
>
> Version: V1.5.29.0 Release · Tech Stack: WPF + .NET Framework 4.x + C# 5 + PowerShell 5.1
>
...
The main window (MainWindow) serves as the unified shell, hosting the starfield control, main view host, and privilege indicator. The AuroraPrivilegeIndicator in the upper-right corner intuitively shows the current process privilege level with a green shield (administrator) or orange shield (normal user), along with bilingual labels.
...
**Starfield background (AuroraStarfield)** is the visual foundation of the entire UI. Star count is differentiated by tier: Eco=80, Balanced=180, Performance=350, Extreme=600. The background includes 5 aurora curtain layers (AuroraCurtain), each with an independent Y position, hue (Hue), and max alpha: Layer 0 at Y=0.12 shows green (Hue=140, MaxAlpha=145), Layer 1 at Y=0.28 shows cyan (Hue=185, MaxAlpha=120), Layer 2 at Y=0.48 shows purple (Hue=270, MaxAlpha=100), Layer 3 at Y=0.68 shows magenta (Hue=320, MaxAlpha=85), Layer 4 at Y=0.85 shows green again (Hue=160, MaxAlpha=70). Meteors, 4-corner cross starbursts (Diffraction), and aurora Bloom glow are enabled only in Performance / Extreme tiers. Mouse movement triggers constellation lines (Connections, 30-frame throttle) and mouse glow (GlowFactor). The starfield also implements a multi-level depth parallax system (V1.5.28.9), mapping different views to 6 depth levels (Splash=0.0 → SolutionDetail=1.0), with smooth transitions via UwpStandardEase curves when switching views, avoiding abrupt level jumps. All animations are driven by CompositionTarget.Rendering (replacing DispatcherTimer), and use sine lookup tables (4096 entries + linear interpolation) and Brush caching to achieve zero-allocation hot paths.
...
*AURORA VelociRaptor-GR Dev PRJ. · V1.5.29.0 Release*
