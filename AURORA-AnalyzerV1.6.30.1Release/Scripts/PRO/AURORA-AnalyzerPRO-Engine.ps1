...
$Language      = if ($script:PRO_Language)       { $script:PRO_Language }       else { "CHS" }
$OutputPath    = if ($script:PRO_OutputPath)     { $script:PRO_OutputPath }     else { $null }
$AutoOpen      = if ($script:PRO_AutoOpen)       { $script:PRO_AutoOpen }       else { $false }
$LogType       = if ($script:PRO_LogType)        { $script:PRO_LogType }        else { "System" }
$EventId       = if ($script:PRO_EventId)        { $script:PRO_EventId }        else { "" }
$ProviderName  = if ($script:PRO_ProviderName)   { $script:PRO_ProviderName }   else { "" }
$Level         = if ($script:PRO_Level)          { $script:PRO_Level }          else { "" }
$StartTime     = if ($script:PRO_StartTime)      { $script:PRO_StartTime }      else { (Get-Date).Date }
$EndTime       = if ($script:PRO_EndTime)        { $script:PRO_EndTime }        else { (Get-Date).Date.AddDays(1) }
$ForceRescan   = if ($script:PRO_ForceRescan)    { $script:PRO_ForceRescan }    else { $false }
$ExportMode    = if ($script:PRO_ExportMode)     { $script:PRO_ExportMode }     else { "SingleDay" }
$ExportScope   = if ($script:PRO_ExportScope)    { $script:PRO_ExportScope }    else { "Full" }
$TrendAnalysis = if ($script:PRO_TrendAnalysis)  { $script:PRO_TrendAnalysis }  else { $false }
$GUI_Mode      = if ($script:PRO_GUI_Mode)       { $script:PRO_GUI_Mode }       else { $false }
$Silent        = if ($script:PRO_Silent)         { $script:PRO_Silent }         else { $false }
...
# ==========================================
# 注意：CoreEngine 必须在启动检测前加载，因为 Invoke-SafeExit 定义在其中
...
# 通用StreamWriter处理函数
function New-PROStreamWriterOperation {
    <#
    .SYNOPSIS
        通用StreamWriter操作函数
    .DESCRIPTION
    ...
    .EXAMPLE
        ...
        $result = New-PROStreamWriterOperation -FilePath "C:\temp\output.txt" -ScriptBlock {
            param($writer)
...
function Get-PROSafeFilePath {
    ...
    .EXAMPLE
        ...
        $safePath = Get-PROSafeFilePath -BasePath $basePath -LogType $logType -DatePart $datePart -Extension $extension
...
function Write-CustomProgress {
    ...
    if (Get-Variable -Name "syncHash" -Scope Global -ErrorAction SilentlyContinue) {
        ...
        # [cache.txt 修复] 计算速度/ETA 并写入 ProgressDetail，让 WPF 版也能显示原版 ConsoleHost 的丰富进度信息
        if ($TotalItems -gt 0 -and $PercentComplete -gt 0 -and $PercentComplete -lt 100) {
            # 初始化此活动的进度跟踪（与 ConsoleHost 分支共享 $script:progressStartTime）
            ...
            if ($elapsedSeconds -gt 1) {
                ...
                if ($estimatedRemainingSeconds -gt 0) {
                    ...
                    $etaStr = $timespan.ToString() -replace '\.[0-9]+', ''
                    $global:syncHash['ProgressDetail'] = "$($speed)项/秒 | 剩余$etaStr"
                }
            }
        }
        elseif ($PercentComplete -ge 100) {
            $global:syncHash['ProgressDetail'] = ""
        }
    }
    # ==================================
    # 检查是否在控制台中运行
...
function New-LogReport {
    ...
    try {
        # 优化：使用通用函数生成安全的文件路径
        $txtPath = Get-PROSafeFilePath -BasePath $ExportPath -LogType $LogType -DatePart $DatePart -Extension "_摘要.txt"
        ...
        $csvPath = Get-PROSafeFilePath -BasePath $ExportPath -LogType $LogType -DatePart $DatePart -Extension ".csv"
        
        # 优化：使用通用StreamWriter操作函数
        $csvSuccess = New-PROStreamWriterOperation -FilePath $csvPath -ScriptBlock {
            param($streamWriter)
            ...
            # [BUG 修复] 之前的 ForEach-Object 管道有 PowerShell 作用域问题：
            #   $csvLines += $csvLine 在 ForEach-Object 内部创建局部变量，
            #   父作用域的 $csvLines 永远是 @()，导致 CSV 只写入最后 1 行。
            #   改为简单 foreach 顺序写入，每行直接 WriteLine，彻底消除作用域问题。
            $totalProcessed = 0
            $startTime = Get-Date
        ...
        # 导出为JSON格式
        $jsonPath = Get-PROSafeFilePath -BasePath $ExportPath -LogType $LogType -DatePart $DatePart -Extension ".json"
        
        # 优化：使用通用StreamWriter操作函数
        $jsonSuccess = New-PROStreamWriterOperation -FilePath $jsonPath -ScriptBlock {
            param($streamWriter)
        ...
        # 导出为XML格式
        $xmlPath = Get-PROSafeFilePath -BasePath $ExportPath -LogType $LogType -DatePart $DatePart -Extension ".xml"
        
        # 优化：使用通用StreamWriter操作函数
        $xmlSuccess = New-PROStreamWriterOperation -FilePath $xmlPath -ScriptBlock {
            param($streamWriter)
    ...
    for ($scopeIndex = 0; $scopeIndex -lt $scopes.Count; $scopeIndex++) {
        ...
        if ($restoreMode -and $restoredSession.Progress -ge 70) {
            ...
            if ($restoredSession.Data.HealthScore) {
                ...
                # 根据健康等级设置颜色
                if ($healthLevel -eq $script:Loc['Health_Excellent']) {
                    $consoleColor = 'Green'
                } elseif ($healthLevel -eq $script:Loc['Health_Good']) {
                    $consoleColor = 'Yellow'
                } elseif ($healthLevel -eq $script:Loc['Health_Fair']) {
                    $consoleColor = 'Yellow'
                } else {
                    $consoleColor = 'Red'
                }
        ...
        # [问题5修复] 将统计字段写入 syncHash，供 C# OnScriptComplete 读取并填充 metadata。
        #   根因：C# ProModeViewModel.OnScriptComplete 通过 _psHost.GetSyncHashValue 读取这些字段，
        #   但 PowerShell 此前仅通过 Save-PROProgress -AdditionalData 写入，C# 读不到，
        #   导致 .metadata.json 中 HealthScore/CriticalEvents 等为 0，报告显示不正确。
        try {
            $global:syncHash['HealthScore'] = $healthScore
            $global:syncHash['HealthLevel'] = $healthLevel
            $global:syncHash['HealthStatus'] = $healthStatus
            $global:syncHash['CriticalEvents'] = $critical
            $global:syncHash['ErrorEvents'] = $errors
            $global:syncHash['WarningEvents'] = $warnings
            $global:syncHash['TotalEventCount'] = $totalEventCount
            $global:syncHash['TotalEvents'] = $totalEventCount
            # DateRange 由 Scopes 拼接（兼容单范围/多范围）
            if ($null -ne $scope -and $null -ne $scope.StartTime -and $null -ne $scope.EndTime) {
                $global:syncHash['DateRange'] = "$($scope.StartTime) to $($scope.EndTime)"
            }
        } catch {
            # 静默失败：syncHash 写入失败不影响主流程
        }
        }
        # === 【断点续传修复：健康评估结束】===
