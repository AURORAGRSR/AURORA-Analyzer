...
# 所有函数内部保留 [CmdletBinding()] 以支持参数绑定
...
$script:ActiveDir = $null
$script:CheckpointsDir = $null
$script:ArchiveDir = $null
$script:CurrentSessionId = $null
$script:SessionData = @{}
$script:IsInitialized = $false
$script:Language = "CHS"  # 默认中文，支持 CHS/ENG
...
function Get-LocalizedString {
    ...
    if ($strings.ContainsKey($Key)) {
        ...
        $text = if ($langData.ContainsKey($script:Language)) { $langData[$script:Language] } else { $langData["CHS"] }
        ...
        try {
            ...
            if ($ToolPath -and (Test-Path $ToolPath)) {
                ...
                try {
                    ...
                    # 工具目录可用
                    $script:CacheRoot = $primaryCache
                    $script:ActiveDir = Join-Path $primaryCache "active"
                    $script:CheckpointsDir = Join-Path $primaryCache "checkpoints"
                    $script:ArchiveDir = Join-Path $primaryCache "archive"
            ...
            $script:CacheRoot = $tempCache
            $script:ActiveDir = Join-Path $tempCache "active"
            $script:CheckpointsDir = Join-Path $tempCache "checkpoints"
            $script:ArchiveDir = Join-Path $tempCache "archive"
...
function Save-SessionProgress {
    ...
    process {
        ...
        try {
            # 更新会话数据
            $script:SessionData.LastUpdated = Get-Date -Format "o"
            
            if ($Stage) {
                $script:SessionData.CurrentStage = $Stage
            }
            
            # 只有当Progress >= 0时才更新（-1表示不更新）
            if ($Progress -ge 0) {
                $script:SessionData.Progress = $Progress
            }
            ...
            while (-not $saved -and $retryCount -lt $maxRetries) {
                try {
                    ...
                    # 【Phase 4.1 原子写入】原子替换：删除旧文件，重命名临时文件
                    ...
                    # 【Phase 4.1 原子写入】重命名临时文件到目标文件（原子操作）
                    [System.IO.File]::Move($tempFile, $activeFile)
                    
                    $saved = $true
                    Write-Verbose (Get-LocalizedString "SavingProgress" @($Stage, $Progress, $retryCount, $maxRetries))
                    ...
                    Write-Verbose (Get-LocalizedString "SaveFailed" @($retryCount, $maxRetries, $_.Exception.Message))
            ...
            if (-not $saved) {
                $errorMsg = if ($script:Language -eq "ENG") { "Failed to write session file after $maxRetries attempts" } else { "无法写入会话文件（已尝试 $maxRetries 次）" }
                throw $errorMsg
            }
            
            # 创建检查点
            if ($CreateCheckpoint) {
                $checkpointFile = Join-Path $script:CheckpointsDir "${SessionId}_$(Get-Date -Format 'yyyyMMdd_HHmmss').json"
                try {
                    $jsonContent = $script:SessionData | ConvertTo-Json -Depth 10
                ...
                # 添加到检查点列表
                $script:SessionData.Checkpoints += @{
                    CreatedAt = Get-Date -Format "o"
                    Stage = $Stage
                    File = $checkpointFile
                }
    ...
    process {
        try {
            ...
            foreach ($prop in $sessionData.PSObject.Properties) {
                $script:SessionData[$prop.Name] = $prop.Value
            }
            
            return @{
                SessionId = $script:SessionData.SessionId
                Stage = $script:SessionData.CurrentStage
                Progress = $script:SessionData.Progress
                LastUpdated = $script:SessionData.LastUpdated
                Data = $script:SessionData
    ...
    process {
        try {
            ...
            if ($Archive -and (Test-Path $activeFile)) {
                # 归档：移动到 archive 目录
                $archiveFile = Join-Path $script:ArchiveDir "${SessionId}_$(Get-Date -Format 'yyyyMMdd_HHmmss').json"
            ...
            # 清理检查点
            if (-not $Archive) {
                $checkpointPattern = Join-Path $script:CheckpointsDir "${SessionId}_*.json"
            ...
            # 重置当前会话
            if ($SessionId -eq $script:CurrentSessionId) {
                $script:CurrentSessionId = $null
                $script:SessionData = @{}
            }
        }
...
function Get-LatestPendingSession {
    ...
    process {
        try {
            # 查找所有活动会话
            $sessionFiles = Get-ChildItem -Path $script:ActiveDir -Filter "*.json" -ErrorAction SilentlyContinue |
...
function Complete-Session {
    ...
    process {
        try {
            ...
            $keysToRemove = @()
            foreach ($key in $script:SessionData.Keys) {
                $value = $script:SessionData[$key]
                if ($value -eq $null) {
                    $keysToRemove += $key
    ...
    process {
        try {
            ...
            if (Test-Path $script:ActiveDir) {
                $oldSessions = Get-ChildItem -Path $script:ActiveDir -Filter "*.json" |
            ...
            # 清理过期的检查点
            if (Test-Path $script:CheckpointsDir) {
                $oldCheckpoints = Get-ChildItem -Path $script:CheckpointsDir -Filter "*.json" |
                    Where-Object { $_.LastWriteTime -lt $cutoffDate }
            ...
            if (Test-Path $script:ArchiveDir) {
                $oldArchives = Get-ChildItem -Path $script:ArchiveDir -Filter "*.json" |
...
function Get-SessionStatistics {
    ...
    process {
        try {
            ...
            $errorMsg = if ($script:Language -eq "ENG") { "Failed to get session statistics: $($_.Exception.Message)" } else { "获取统计信息失败：$($_.Exception.Message)" }
