...
if (-not $isLaunchedByGUI) {
    ...
    Write-Host "Double-click AURORA.Launcher-双击启动.exe" -ForegroundColor White
    Write-Host ""
...
# ==========================================
# 🔐 C# Runtime Integrity Guard (Compiled to IL, visible across Runspaces)
# ==========================================
# Note: AuroraGuard is compiled in main Runspace, may not be accessible in child Runspaces
# If type doesn't exist, it was already verified in LauncherGUI, skip this check
try {
    ...
    # Type not found or verification failed, silently skip
}
    ...
    if (-not $Silent) {
        ...
        if ($restoredSession) {
            ...
            if ($GUI_Mode -or (Get-Variable -Name "syncHash" -Scope Global -ErrorAction SilentlyContinue)) {
                # Ensure syncHash is a hashtable
                if ($null -ne $global:syncHash -and $global:syncHash -is [System.Collections.IDictionary]) {
                    ...
                    # syncHash is not a valid hashtable, fallback to console mode
                    Write-Host "⚠️ syncHash invalid, using console mode" -ForegroundColor Yellow
                    $restoreMode = $false
                }
            } else {
                # Non-GUI mode: console selection prompt
                Write-Host "⚠️ Detected incomplete session, assuming restart in console mode" -ForegroundColor Yellow
                $restoreMode = $false
            }
            
            # Check user choice (only when syncHash is valid)
            if ($null -ne $global:syncHash -and $global:syncHash -is [System.Collections.IDictionary]) {
...
if (-not $restoreMode) {
    ...
    Save-ENGProgress -SessionId $sessionId -Checkpoint "Initialized" -AdditionalData @{
        LogType = $LogType
        ExportMode = $ExportMode
        ExportScope = $ExportScope
    }
...
try {
    ...
    elseif ($Silent) {
        ...
        Save-ENGProgress -SessionId $sessionId -Checkpoint "LogTypeSelected" -AdditionalData @{
    ...
    Save-ENGProgress -SessionId $sessionId -Checkpoint "PerformanceAssessed" -AdditionalData @{
        PerformanceScore = $performanceScore
    ...
    for ($scopeIndex = 0; $scopeIndex -lt $scopes.Count; $scopeIndex++) {
        ...
        if ($scopes.Count -gt 1) {
            ...
            Save-ENGProgress -SessionId $sessionId -Checkpoint "ProcessingStarted" -CustomProgress $currentProgress -CustomStage "Processing $($scope.LogType) ($($scopeIndex + 1)/$($scopes.Count))"
        }
    ...
    $scanProgress = 60
    Save-ENGProgress -SessionId $sessionId -Checkpoint "HighRiskScanComplete" -CustomProgress $scanProgress -CustomStage "High-risk event scan complete, found $totalHigh high-risk events" -AdditionalData @{
    ...
    $healthProgress = 70
    Save-ENGProgress -SessionId $sessionId -Checkpoint "HealthAssessmentComplete" -CustomProgress $healthProgress -CustomStage "System health assessment complete" -AdditionalData @{
        HealthScore = $healthScore
    ...
    if ((-not $restoreMode) -or ($restoredSession.Progress -lt 80)) {
        if ($exportChoice -eq "HighRisk") {
            ...
            Save-ENGProgress -SessionId $sessionId -Checkpoint "ExportModeSelected" -CustomProgress 75 -CustomStage "Export mode selected: $reportMode" -AdditionalData @{
                ExportChoice = $exportChoice
                ReportMode = $reportMode
            }
        } else {
            # Save progress: Fetching full log
            Save-ENGProgress -SessionId $sessionId -Checkpoint "FetchingFullLog" -CustomProgress 75 -CustomStage "Fetching full $($scope.LogType) log" -AdditionalData @{
                ExportChoice = $exportChoice
                ReportMode = "Full $($scope.LogType) Log"
            }
            ...
            Save-ENGProgress -SessionId $sessionId -Checkpoint "FullLogFetched" -CustomProgress 80 -CustomStage "Full $($scope.LogType) log fetched, $($events.Count) events" -AdditionalData @{
                ExportChoice = $exportChoice
                ReportMode = $reportMode
    ...
    Save-ENGProgress -SessionId $sessionId -Checkpoint "ExportStarted" -CustomProgress 85 -CustomStage "Exporting: $reportMode"
    ...
    if ($Silent) {
        ...
        if ($trendAnalysisChoice -like 'Y*') {
            ...
            if ($scopeIndex -eq $scopes.Count - 1) {
                Save-ENGProgress -SessionId $sessionId -Checkpoint "TrendAnalysisComplete"
            }
        }
        }
    }
    ...
    if ((-not $restoreMode) -or ($restoredSession.Progress -lt 95)) {
        Save-ENGProgress -SessionId $sessionId -Checkpoint "SmartAnalysisPending" -AdditionalData @{
            ExportPath = $outDir
        }
    }
        ...
        try {
            ...
            if ($userConfirmed) {
                ...
                if (Test-Path $smartEnginePath) {
                    ...
                    Save-ENGProgress -SessionId $sessionId -Checkpoint "SmartAnalysisComplete"
                } else {
    ...
    if (-not $Silent) {
        ...
        # Save final progress
        Save-ENGProgress -SessionId $sessionId -Checkpoint "Completed"
