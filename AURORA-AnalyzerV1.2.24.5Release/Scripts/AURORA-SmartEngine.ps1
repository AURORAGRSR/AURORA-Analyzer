...
# 检测方式 3: 检查是否通过 hash 参数传入（仅限内部，需要 syncHash 已存在）
if (-not $isLaunchedByGUI -and $null -ne $hash -and $null -ne $global:syncHash) {
    $isLaunchedByGUI = $true
}

# 检测方式 4: RSA 令牌验证（由合法 EXE 签发的令牌）
...
if ($null -eq $global:syncHash) {
    ...
    Write-Host "  1. 双击运行 AURORA.Launcher-双击启动.exe" -ForegroundColor Gray
...
function Invoke-AuroraSafeAction {
    ...
    if ($Command.elevation_required -eq $true -and -not $isAdmin) {
        ...
        if ($global:syncHash.ElevationAuthorized -eq $true) {
            ...
            $errorMsg = if ($Language -eq "CHS") {
                "❌ 需要管理员权限！请以管理员身份重新运行 AURORA。\n提示：右键点击 AURORA.Launcher-双击启动.exe，选择'以管理员身份运行'"
            } else {
                "❌ Administrator privileges required! Please re-run AURORA as administrator.\nHint: Right-click AURORA.Launcher-双击启动.exe and select 'Run as administrator'"
            }
    ...
    if ($isRepairCommand -and $hasSideEffects) {
        ...
        Write-SmartLog ""
        if ($Language -eq "CHS") {
            Write-SmartLog "📦 Phase 4.2: 正在创建撤销保护（Undo Support）..." -ForegroundColor Cyan
        } else {
            Write-SmartLog "📦 Phase 4.2: Creating Undo protection..." -ForegroundColor Cyan
        }
        ...
        try {
            ...
            if ($repairSession) {
                ...
                if ($Language -eq "CHS") {
                    Write-SmartLog "   ✅ 修复会话已创建：$repairSessionId" -ForegroundColor Green
                } else {
                    Write-SmartLog "   ✅ Repair session created: $repairSessionId" -ForegroundColor Green
                }
            }
            ...
            if ($needsBackup) {
                ...
                if ($paths.Count -gt 0) {
                    ...
                    if ($snapshot) {
                        ...
                        if ($Language -eq "CHS") {
                            Write-SmartLog "   ✅ 快速备份已创建：$backupSnapshotId (大小：$($snapshot.Size))" -ForegroundColor Green
                        } else {
                            Write-SmartLog "   ✅ Fast backup created: $backupSnapshotId (Size: $($snapshot.Size))" -ForegroundColor Green
                        }
                    }
                }
            }
            ...
            if ($repairSessionId) {
                ...
                if ($canUndo) {
                    # 完成会话并标记为可撤销
                    Complete-RepairSession -SessionId $repairSessionId -Status "Success" -BackupSnapshotId $backupSnapshotId
                    
                    if ($Language -eq "CHS") {
                        Write-SmartLog "   🔄 撤销保护已就绪（可撤销此修复操作）" -ForegroundColor Cyan
                    } else {
                        Write-SmartLog "   🔄 Undo protection ready (this repair can be undone)" -ForegroundColor Cyan
                    }
                } else {
                    Complete-RepairSession -SessionId $repairSessionId -Status "Success"
                }
            }
            ...
            if ($Language -eq "CHS") {
                Write-SmartLog "   ⚠️ 撤销保护创建失败：$($_.Exception.Message)" -ForegroundColor Yellow
            } else {
                Write-SmartLog "   ⚠️ Undo protection creation failed: $($_.Exception.Message)" -ForegroundColor Yellow
            }
        }
    ...
    if ((Test-Path $cachePath) -and 
        ...
        try {
            ...
            if ($cachedData.FlatRules -and 
                ...
                $cachedData.KbVersion -eq $techData.meta.version) {
