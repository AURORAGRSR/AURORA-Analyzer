...
# LauncherGUI Version: V1.2.20.5Release
# 作者：AURORA VelociRaptor-GR Dev PRJ.| 构建时间：2026.06.01
...
<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue>
'@

$global:AURORA_SessionSalt = [Convert]::FromBase64String('vuTYWi3O5xGCpYTBUOIrVc5fsxboThE4X6obh904khA=')
...
function Decrypt-HashListFromToken {
    ...
    try {
        ...
        Write-Host "Hash list decryption failed: $($_.Exception.Message)" -ForegroundColor Red
        return $null
    }
}
...
. "$scriptDir\AURORA-GUI-Functions.ps1"
...
if (-not [string]::IsNullOrWhiteSpace($TokenPath) -and (Test-Path $TokenPath)) {
    ...
    try { Remove-Item $TokenPath -Force -ErrorAction SilentlyContinue } catch {}
}
...
if ($ElevationTokenPath -and (Test-Path $ElevationTokenPath)) {
    try {
        ...
        if ($elevParts.Count -eq 3) {
            ...
            if ($elevAge -gt 0 -and $elevAge -lt 120) {
                ...
                Write-Host "[安全] 提权令牌已过期（时效：$elevAge 秒，最大允许：120秒）" -ForegroundColor Yellow
            }
        }
    } catch {
    ...
    try { Remove-Item $ElevationTokenPath -Force -ErrorAction SilentlyContinue } catch {}
}
    ...
    $cleanupAction = {
        ...
        try {
            if ($AURORA_WD_PIPE -ne $null) {
                try { if ($AURORA_WD_PIPE.IsConnected) { $AURORA_WD_PIPE.Close() }; $AURORA_WD_PIPE.Dispose() } catch {}
            }
            if ($AURORA_WD_PS -ne $null) {
                try { if (-not $AURORA_WD_PS.InvocationStateInfo.Completed) { $AURORA_WD_PS.Stop() }; $AURORA_WD_PS.Dispose() } catch {}
            }
            if ($AURORA_WD_RUNSPACE -ne $null) {
                try { if ($AURORA_WD_RUNSPACE.RunspaceStateInfo.State -eq 'Opened') { $AURORA_WD_RUNSPACE.Close() }; $AURORA_WD_RUNSPACE.Dispose() } catch {}
            }
            if ($global:ActivePS -ne $null) {
                try { if (-not $global:ActivePS.InvocationStateInfo.Completed) { $global:ActivePS.Stop() }; $global:ActivePS.Dispose() } catch {}
            }
            if ($global:ActiveRunspace -ne $null) {
                try { if ($global:ActiveRunspace.RunspaceStateInfo.State -eq 'Opened') { $global:ActiveRunspace.Close() }; $global:ActiveRunspace.Dispose() } catch {}
            }
            if ($script:debuggerTimer -ne $null) {
                try { $script:debuggerTimer.Enabled = $false; $script:debuggerTimer.Stop(); $script:debuggerTimer.Dispose() } catch {}
            }
            if ($script:debuggerWmiWatcher -ne $null) {
                try { Unregister-Event -SourceIdentifier $script:debuggerWmiWatcher.Name -ErrorAction SilentlyContinue } catch {}
            }
    ...
    while ($AURORA_WD_RETRY_COUNT -lt $AURORA_WD_MAX_RETRY) {
        try {
            ...
            if ($AURORA_WD_RETRY_COUNT -lt $AURORA_WD_MAX_RETRY) {
                ...
                Write-Host "[看门狗] 已达到最大重试次数，将以独立模式运行" -ForegroundColor Red
                $AURORA_WATCHDOG_ACTIVE = $false
                Clear-AuroraWatchdogEnv
            }
        }
...
public class AuroraGuard
{
    ...
    {
        { "Scripts\\AURORA-SmartEngine.ps1", "1643b307debb1c670c828658d4a576ca9928224f734e90f5cef02fb3902d7190" },
        { "Scripts\\AURORA-CoreEngine.ps1", "5498a40541a6f20a518e59f294bef279d9a60f6ab5355cc6f7dfbd6401babf07" },
        { "Scripts\\AURORA-AnalyzerCHSPRO.ps1", "e48c42bf095dc62615d62a0d3febab4c86c3ab6c29be222e60b98dd8925f9402" },
        { "Scripts\\AURORA-ProgressManager.ps1", "cc43e4368cf1bacafcdbd95b86216b12170685a18808de0278a21f6696a803d4" },
        { "Scripts\\AURORA-GUI-Functions.ps1", "9a28bd0c0a20e7b191f5a96ec89701cbd52126de154251008f5a3847c93d769e" },
        { "Scripts\\AURORA-RepairTools.ps1", "aee0f77e9bd4555e68ffd6b913291a96c2967439fe1a852a7d92103c9debe116" },
        { "Scripts\\AURORA-UndoManager.ps1", "fd2a935e150bf46bf964930718eb5307c1ad24948f37b6b3d42316893f89699a" },
        { "Scripts\\AURORA-RestoreManager.ps1", "b844745e027e49d9e52b83933d008e07ba1f08cbefbb1b936409f2b2eb1b801f" },
        { "Scripts\\AURORA-RepairLogger.ps1", "50ac6a550be5160d31e2d8b3f6201e2c06430e0cfdae1388122f8a4d33a7288b" },
        { "Scripts\\AURORA-UndoViewer.ps1", "6d597f66837f7b8a784839f3b0858dfeebe463eba2d3e3d29dc8e6ce67fe2404" },
        { "Scripts\\AURORA-AnalyzerPRO.ps1", "f1ec0aa2338d1029a12dfe90aeb7ce5e52f93a32208777f72f3e0a29b5b09974" },
        { "Scripts\\AURORA-ProgressManager-Integration.ps1", "06965e244f6547b7b8568a7e00ef7c3c0b6a3d95fb00dac1bfdc7e13d7d60703" },
        { "Scripts\\AURORA-ProgressManager-Integration-CHS.ps1", "5c80eccd230c716568ea95ab1b7d26f8dc70db31ce765f10623eab1b9f1042dc" },
        { "Scripts\\AURORA-ProgressManager-Integration-ENG.ps1", "d2f838543441a7a291881e86ccc6389116fdac9e9a3742b1b2a5f8eb817b2f2a" },
        { "Scripts\\Core\\AURORA-AnimationCoreEngine.ps1", "9fb16e3be1e3b6084c630f5178ddf4f93a11cc0e6a06474b558508361c7ffb81" },
...
if (-not $IsLaunchedByExe) {
    ...
    try {
        ...
        # 关键修复：将解密后的 hash 列表保存到全局变量，供后续完整性检查使用
...
$global:IntegrityCheckInterval = 3000   # 🔐 P0 修复：从 10 秒改为 3 秒，进一步减少攻击窗口
...
function Test-FileIntegrity {
    ...
    $failedFiles = @()
    $script:ExpectedFileHashes = @{}  # 关键修复：使用 script 作用域而非 global
...
function Initialize-RuntimeIntegrityCheck {
    ...
    # 初始化文件最后检查时间哈希表（用于 LastWriteTime 快速筛选）
    ...
    # 关键修复：使用 script 作用域变量，确保闭包能正确访问
    ...
    # 🔐 P0 修复：添加随机检查 Timer - 增加攻击难度 (缩短到 2-7 秒)
    $script:randomIntegrityTimer = New-Object System.Windows.Forms.Timer
    $randomInterval = Get-Random -Minimum 2000 -Maximum 7000  # 2-7 秒随机
    $script:randomIntegrityTimer.Interval = $randomInterval
    
    # 🔐 P0 修复：添加 FileSystemWatcher 实时监控
    $script:fileWatcher = New-Object System.IO.FileSystemWatcher
    $script:fileWatcher.Path = $script:CapturedRootDir
    $script:fileWatcher.Filter = "*.ps1,*.json,*.xml,*.ico,*.exe,*.enc"
    $script:fileWatcher.IncludeSubdirectories = $true
    $script:fileWatcher.EnableRaisingEvents = $true
    $script:fileWatcher.NotifyFilter = [System.IO.NotifyFilters]::FileName -bor [System.IO.NotifyFilters]::Size -bor [System.IO.NotifyFilters]::LastWrite
    
    # 创建共享的检查逻辑函数 (使用 script 作用域确保 Timer 能访问)
    $script:checkIntegrity = {
        ...
        # 🔐 P0 修复 2: 移除 break 语句，检查所有文件并收集所有问题
        foreach ($file in $script:ExpectedFileHashes.Keys) {
            $fullPath = Join-Path $script:CapturedRootDir $file
            if (-not (Test-Path $fullPath)) { 
                # 文件不存在时立即标记为篡改，继续检查其他文件
                $tampered = $true
                if ($missingFiles -eq $null) { $missingFiles = @() }
                $missingFiles += $file
            } else {
                # P2 修复：先检查 LastWriteTime 作为快速筛选，只有修改时间变化才计算 SHA256
        ...
        if ($tampered) {
            ...
            if ($script:fileWatcher) { $script:fileWatcher.EnableRaisingEvents = $false }
            
            # 🔐 安全增强：先隐藏所有窗口，再弹出警告窗口
            if ($global:proForm) {
                try {
    ...
    # 🔐 P0 修复 4: 绑定 FileSystemWatcher 事件处理程序 (增强版)
    $fileChangedAction = {
        param($source, $e)
        # 文件变化时立即触发完整性检查 (减少等待时间)
        Start-Sleep -Milliseconds 200  # 等待文件操作完成
        Write-Host "[FileSystemWatcher] 检测到文件变化：$($e.Name) - $($e.ChangeType)" -ForegroundColor Yellow
        & $script:checkIntegrity @($null, $null)
    }
    
    $script:fileWatcher.Add_Changed($fileChangedAction)
    $script:fileWatcher.Add_Deleted($fileChangedAction)
    $script:fileWatcher.Add_Renamed({
        param($source, $e)
        # 文件重命名/移动时也触发检查
        Start-Sleep -Milliseconds 200
        Write-Host "[FileSystemWatcher] 检测到文件重命名/移动：$($e.OldName) -> $($e.Name)" -ForegroundColor Yellow
        & $script:checkIntegrity @($null, $null)
    })
    $script:fileWatcher.Add_Created({
        param($source, $e)
        # 新文件创建时也检查（防止注入恶意文件）
        Start-Sleep -Milliseconds 500
        $fullPath = $e.FullPath
        $relativePath = $fullPath.Replace($script:CapturedRootDir, "").TrimStart('\')
        
        # 检查新文件是否在白名单中
        if (-not $script:ExpectedFileHashes.ContainsKey($relativePath)) {
            if ($script:ExitCountdownStarted) { return }
            $script:ExitCountdownStarted = $true
    ...
    # 🔐 P1 修复：启动后立即执行一次完整性检查 (填补启动窗口漏洞)
    # 使用 PowerShell 定时器，1 秒后执行首次检查
    $startupCheckTimer = New-Object System.Windows.Forms.Timer
    $startupCheckTimer.Interval = 1000  # 1 秒后执行
    ...
    # 保存清理逻辑到 script 作用域，供 ShowMainForm 绑定到 FormClosing 事件
...
if ($null -ne $global:PassedHashListFromExe -and $global:PassedHashListFromExe.Length -gt 0) {
    ...
    if (-not $integrityResult.Success) {
        ...
        if ($env:AURORA_EXE_VERIFIED -eq "1") {
            ...
            Write-Host "[警告] 请通过 AURORA.Launcher-双击启动.exe 启动以确保完整性" -ForegroundColor Yellow
        }
    }
    ...
    if ($env:AURORA_EXE_VERIFIED -eq "1") {
        ...
        Write-Host "[注意] 建议通过 AURORA.Launcher-双击启动.exe 启动以获得安全保障" -ForegroundColor Yellow
    }
}
...
function Test-DirectoryIntegrity {
    ...
    if ($null -eq $RequiredFiles -or $null -eq $OptionalFiles) {
        if ($UseChineseUI) {
            $msg = "❌ 完整性配置缺失！RequiredFiles 或 OptionalFiles 未定义。"
        } else {
            $msg = "❌ Integrity configuration missing! RequiredFiles or OptionalFiles not defined."
        }
        [System.Windows.Forms.MessageBox]::Show($msg, "AURORA 启动错误", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        return $false
    }
...
function ShowMainForm {
            ...
            $launchTimer.Add_Tick({
                ...
                try {
                    ...
                    if ($script:selectedMode -eq "SMART") {
                        ...
                        try { ShowProMode -ScriptPath $engScript -Title "English Mode" -Language "ENG" } catch { Write-Host "[ShowProMode ERROR] $($_.Exception.Message)" -ForegroundColor Red; Write-Host "[ShowProMode Stack] $($_.ScriptStackTrace)" -ForegroundColor Red }
                        ...
                        Write-Host "[ERROR] No valid script found for launch!" -ForegroundColor Red
                        Write-Host "[ERROR] selectedLanguage: $($script:selectedLanguage)" -ForegroundColor Red
                        Write-Host "[ERROR] chsScript null: $($chsScript -eq $null)" -ForegroundColor Red
                        Write-Host "[ERROR] engScript null: $($engScript -eq $null)" -ForegroundColor Red
                    }
    ...
    if ($script:cleanupAction) {
        $form.Add_FormClosing($script:cleanupAction)
    }
        ...
        if ($global:syncHash.RequiresElevation -eq $true) {
            ...
            if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
                ...
                try {
                    ...
                    # 构建启动参数
                    $scriptPath = Join-Path $PSScriptRoot "AURORA-AnalyzerLauncherGUI.ps1"
